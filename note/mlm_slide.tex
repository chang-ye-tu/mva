...
\fancyhead[L]{\small\color{hdrBG}\textbf{Modern Linear Models}}
...
\]
For $p \geqslant 2$, we use the bound $\Gamma(\alpha) \leqslant \alpha^{\alpha-1}$, valid for $\alpha \geqslant 1$. (This follows from $\Gamma(n+1) = n! \leqslant n^n$ for integers $n \geqslant 1$, extended to non-integer $\alpha$ via the log-convexity of $\Gamma$, hence $\Gamma(\alpha) \leqslant \alpha^\alpha/\alpha = \alpha^{\alpha-1}$.) Thus $\Gamma(p/2) \leqslant (p/2)^{p/2-1}$, and substituting:
\[
\expc[|X|^p] \leqslant p\,(K_2\sqrt{2})^p\,(p/2)^{p/2-1} = 2\cdot(2K_2^2)^{p/2}\cdot p^{p/2}.
\]
...
\]
For $p \geqslant 2$ and $|t| \leqslant 1/(2CeK)$, write $r := CeK|t| \leqslant 1/2$; since $\sqrt{p} \geqslant 1$, each term is $\leqslant r^p$, and summing the geometric tail keeps the $t^2$ scaling:
\[
\expc[e^{tX}] \leqslant 1 + \sum_{p=2}^\infty r^p = 1 + \frac{r^2}{1-r} \leqslant 1 + 2r^2 \leqslant e^{2r^2} = e^{2C^2e^2K^2t^2}, \quad \text{for } |t| \leqslant \frac{1}{2CeK},
\]
using $r/(1-r) \leqslant 2r$ (as $r \leqslant 1/2$) and $1 + x \leqslant e^x$. This is already of the form $e^{(\text{const})K^2t^2}$ needed below; a crude constant bound $\expc[e^{tX}] < 2$ would not capture the $t^2$ decay as $t \to 0$.
...
\begin{proof}
By the spectral decomposition of the symmetric part $(\bA + \bA')/2$ (whose quadratic form equals $\bA$'s), we may assume $\bA$ is symmetric. Let $\bA = \bU\bLambda\bU'$ with $\bLambda = \diag(\lambda_1, \ldots, \lambda_n)$. Set $\widetilde\bg = \bU'\bg$, which is again $N(\bzero, \bI_n)$ since $\bU$ is orthogonal. Then
\[
...
Under assumptions~\ref{M1}--\ref{M4} there are universal constants $C', c, c_0 > 0$ such that for every $\eta \in [1, \sqrt{n-p}]$, with probability at least $1 - 2e^{-c\eta^2} - 2e^{-c_0 p}$,
\begin{equation}\label{eq:sse-concentration}
\left|\frac{\SSE}{\sigma_\varepsilon^2} - (n-p)\right| \leqslant C'\eta\sqrt{n-p},
\end{equation}
where $\sigma_\varepsilon^2 = \var(\varepsilon_i)$. Consequently $\MSE = \SSE/(n-p)$ satisfies $|\MSE - \sigma_\varepsilon^2| \leqslant C'\eta\,\sigma_\varepsilon^2/\sqrt{n-p}$ on the same event, so at any fixed confidence the deviation is of the central-limit scale $\sqrt{n-p}$ and $\MSE \to \sigma_\varepsilon^2$ at the $1/\sqrt{n-p}$ rate. (This is the correct scaling: a deviation of order $\sqrt{n-p}$ holds only at constant confidence, exactly as $\SSE/\sigma^2 \sim \chi^2_{n-p}$ has standard deviation $\sqrt{2(n-p)}$ under normality; an exponential-in-$p$ confidence $1 - e^{-cp}$ would require the larger deviation of order $\sqrt{p(n-p)}$, a constant relative error in the proportional regime $p \asymp n$.)
\end{theorem}
...
\]
This is~\eqref{eq:sse-concentration} with $\eta := C_0$ (any value in $[1, \sqrt{n-p}]$ keeps $\delta = C_0/\sqrt{n-p} \leqslant 1$, the range used in Step~4). The confidence is governed by the deviation parameter $\eta$, not by $p$: at the central-limit scale $\eta = O(1)$ the failure probability $2e^{-c\eta^2}$ is a small tunable constant, and forcing it down to $e^{-cp}$ would require $\eta \asymp \sqrt{p}$, i.e.\ the larger deviation $\asymp\sqrt{p(n-p)}$. Dividing by $n - p$: $|\MSE - \sigma_\varepsilon^2| \leqslant C'\eta\,\sigma_\varepsilon^2/\sqrt{n-p}$.
\end{proof}
